# cookie和session
      ...
      * pv  page view   网页浏览次数    刷新一次网页  pv+1    
      * uv   unique visitor 独立访客     用cookie为区分   
      * 独立ip   比如千锋教育   我们所有人到12306买票  对于 12306而言  独立IP加1
...
3. cookie和session结合使用：`web`开发发展至今，`cookie`和`session`的使用已经出现了一些非常成熟的方案。在如今的市场或者企业里，一般有两种存储方式：
   ...
   1. 你登陆京东  京东会把你的 用户id 用户名等相关信息 保存在 session中
   2. 同时会将加密后的session_id 发送给浏览器 cookie  cookie中就保存了 session_id 
   3. 当你刷新网页的时候  cookie 会携带者 session_id 到服务器 
   4. 服务器收到session_id 接下来到session查询 是否存在对应的 用户  如果存在 认为你登录了 否则没登陆  
   5. 如果认为你登陆了  从session中取出信息 查询数据库  你好，德宇 
